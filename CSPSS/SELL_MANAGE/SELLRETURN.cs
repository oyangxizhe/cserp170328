...
{
    public partial class SELLRETURN : Form
    {
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(SELLRETURN));
        ...
        public void bind()
        {
            ...
            DataGridViewTextBoxColumn d19 = new DataGridViewTextBoxColumn();
            d19.Name = "累计销退";
            d19.HeaderText = "累计销退";
            ...
            try
            {
                ...
                stb.Append(cSELLRETURN.sql);
                stb.Append("  WHERE  客户名称 LIKE '%" + textBox1.Text + "%'");
                stb.Append("  AND 订单号 LIKE '%" + textBox3.Text + "%'");
        ...
        public void search_o(string sql)
        {
            ...
            else*/ if (v7 == "Y")
            {
                ...
                dt = bc.getdt(sql+" ORDER BY 销退单号 ASC");
 ...
 (SELECT UGID FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" );
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dataGridView1["销退日期", i].Value = dt.Rows[i]["销退日期"].ToString();
                    ...
                    dataGridView1["数量", i].Value = dt.Rows[i]["销退数量"].ToString();
                    dataGridView1["识别码", i].Value = dt.Rows[i]["识别码"].ToString();
                    dataGridView1["金额", i].Value = dt.Rows[i]["金额"].ToString();
                    dataGridView1["订单销退金额", i].Value = dt.Rows[i]["订单销退金额"].ToString();
                    dataGridView1["累计销货", i].Value = dt.Rows[i]["累计销货"].ToString();
                    dataGridView1["累计销退", i].Value = dt.Rows[i]["累计销退"].ToString();
                    

                    dataGridView1["应收", i].Value = dt.Rows[i]["总计"].ToString();
                    dataGridView1["预收", i].Value = dt.Rows[i]["预收金额"].ToString();
                    dataGridView1["实收", i].Value = dt.Rows[i]["实际应收金额"].ToString();
                    dataGridView1["已收", i].Value = dt.Rows[i]["累计收款金额"].ToString();
                    dataGridView1["未收", i].Value = dt.Rows[i]["未收金额"].ToString();
                    dataGridView1["收款记录", i].Value = dt.Rows[i]["收款记录"].ToString();
        ...
        private void dgvStateControl()
        {
            ...
            this.dataGridView1.MergeColumnNames.Add("客户名称");
            this.dataGridView1.MergeColumnNames.Add("订单销退金额");
            this.dataGridView1.MergeColumnNames.Add("总计");
            this.dataGridView1.MergeColumnNames.Add("应收");
            this.dataGridView1.MergeColumnNames.Add("预收");
            this.dataGridView1.MergeColumnNames.Add("实收");
            this.dataGridView1.MergeColumnNames.Add("已收");
            this.dataGridView1.MergeColumnNames.Add("未收");
            this.dataGridView1.MergeColumnNames.Add("收款记录");
            this.dataGridView1.MergeColumnNames.Add("备注");
        ...
        {
            CSPSS.SELL_MANAGE.SELLRETURNT FRM = new SELL_MANAGE.SELLRETURNT(this);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                SELLRETURNT FRM = new SELLRETURNT(this);
                string v1 = dt.Rows[dataGridView1.CurrentCell.RowIndex]["销退单号"].ToString();
                FRM.IDO = v1;
                FRM.ORID = dt.Rows[dataGridView1.CurrentCell.RowIndex]["订单号"].ToString();
