...
{
    public partial class REQUEST_MONEYT : Form
    {
        ...
        REQUEST_MONEY F1= new REQUEST_MONEY();
        ...
        public REQUEST_MONEYT()
        {
            InitializeComponent();
        }
        public REQUEST_MONEYT(REQUEST_MONEY FRM)
        {
            InitializeComponent();
            F1 = FRM;
        }
        private void REQUEST_MONEYT_Load(object sender, EventArgs e)
        {
         
            try
            {
                label14.Text = "(实际应付金额=应付金额-预付金额-扣款金额)";
                ...
                textBox8.BackColor = CCOLOR.YELLOW;
        ...
        protected void bind()
        {
           ...
           dt = ask(textBox1.Text, MGID );
            ...
            dt1 = bc.getdt(cREQUEST_MONEY.sql + " WHERE 应付单号='" + IDO + "'");
            if (dt1.Rows.Count > 0)
            {

                dateTimePicker1.Text = dt1.Rows[0]["应付日期"].ToString();
                textBox5.Text = dt1.Rows[0]["扣款项目"].ToString();
                textBox6.Text = dt1.Rows[0]["扣款金额"].ToString();
                textBox8.Text = dt1.Rows[0]["实际应付金额"].ToString();
                comboBox3.Text = dt1.Rows[0]["预付单号"].ToString();
...
SELECT A.RMID FROM REQUEST_MONEY_MST A 
LEFT JOIN MISC_GODE_MST B ON A.MGID=B.MGID 
WHERE B.PICKID='" + comboBox2.Text + "'");
            MGID = bc.getOnlyString("SELECT MGID FROM MISC_GODE_MST WHERE PICKID='" + comboBox2.Text + "'");
            if (IDO == "")
            {
                hint.Text = "编号不能为空！";
            ...
            else if (bc.exists("SELECT * FROM PAYMENT_ORDER WHERE RMID='" + IDO + "'"))
            {
                hint.Text = "此应付款单存在付款，不允许修改";
                b = true;
            }
            ...
            else if (bc.exists("SELECT * FROM REQUEST_MONEY_MST WHERE MGID='" +MGID  + "'") && IDO !=GET_IDO )
            {
                hint.Text = string.Format ("提货单号: {0} 已经存在应付款记录！",comboBox2.Text );//一个入库单只有一个应付款单
                b = true;
            }
            else if (juage2()==false )
            {
             ...
             else if (comboBox3 .Text  !="" && !bc.exists("SELECT * FROM ADVANCE_PAYMENT  WHERE APID='" + comboBox3 .Text  + "'"))
            {
                hint.Text = "预付款单号不存在于系统中！";
                b = true;
            }
            else if (comboBox3.Text != "" && bc.exists("SELECT * FROM REQUEST_MONEY_MST  WHERE RMID!='" + textBox1.Text +
                "' AND APID='" +comboBox3 .Text  + "'"))
            {
                hint.Text = "预付款单已在其它应付款单中存在";
                b = true;
            }
            ...
            {
                hint.Text = "实际应付金额不能为空！";
                b = true;
            }
            else if (bc.yesno(textBox8.Text) == 0)
            {
                hint.Text = "实际应付金额只能为数字且不能为负数！";
                b = true;
            }
        ...
        private bool juage2()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                {
                    //hint.Text = string.Format("提货单号：{0} 与项次：{1} 销退数量不能大于可销退数量！",
        ...
        private void save()
        {
            ...
            btnSave.Focus();
            cREQUEST_MONEY.MAKERID = LOGIN.USID;
            cREQUEST_MONEY.RMID = IDO;
            cREQUEST_MONEY.REQUEST_MONEY_DATE = dateTimePicker1.Text;
            cREQUEST_MONEY.MGID = bc.getOnlyString("SELECT MGID FROM MISC_GODE_MST WHERE PICKID='"+comboBox2 .Text +"'");
            cREQUEST_MONEY.APID = comboBox3.Text;
            cREQUEST_MONEY.CUTPAYMENT_PROJECT = textBox5.Text;
            cREQUEST_MONEY.CUTPAYMENT_AMOUNT = textBox6.Text;
            cREQUEST_MONEY.INVOICE_NOTAX_AMOUNT = textBox8.Text;
            cREQUEST_MONEY.save(dt);
            IFExecution_SUCCESS = cREQUEST_MONEY.IFExecution_SUCCESS;
            hint.Text = cREQUEST_MONEY.ErrowInfo;
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                     if (bc.exists("SELECT * FROM PAYMENT_ORDER WHERE RMID='" + IDO + "'"))
                     {
                      hint.Text = "此应付款单存在付款，不允许删除";
                     }
                     else
                     {
                         basec.getcoms("DELETE REQUEST_MONEY_MST WHERE RMID='" + textBox1.Text + "'");
                         basec.getcoms("DELETE REQUEST_MONEY_DET WHERE RMID='" + textBox1.Text + "'");
        ...
        {
            /*cREQUEST_MONEY.MAKERID = "";
           
            //cREQUEST_MONEY.ExcelPrint(dt1, "销货单", System.IO.Path.GetFullPath("销货单.xls"));
