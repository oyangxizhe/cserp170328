    ...
    public partial class ORDER : Form
    {
        ...
        public void bind()
        {
            ...
            if (UNAME != null && UNAME != "")
            {
                ...
                stb.Append("  WHERE  C.UNAME LIKE '%" + UNAME  + "%'");
            }
            else
            {
             
                stb.Append("  WHERE  C.UNAME LIKE '%" + textBox1.Text + "%'");
            }
        
            stb.Append(" AND A.ORID LIKE '%" + textBox2 .Text  + "%'");
            string v1 = dateTimePicker1.Text + " 0:00:00";
            string v2 = dateTimePicker2.Text + " 23:59:59";
            if (checkBox1.Checked)
            {
                stb.Append(" AND D.DATE  BETWEEN  '" + v1 + "' AND '" + v2 + "'");
        ...
        public void search_o(string sql)
        {
             ...
             if (v7 == "Y")
            {
                ...
                dt = bc.getdt(sql+" ORDER BY A.ORID ASC");

            }

            else
            {
                dt = bc.getdt(sql + " AND A.MAKERID='" + LOGIN.USID  + "'" );
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (SELECT == 1)
            {
                ...
                SELL_MANAGE.SELLTABLET.IF_DOUBLE_CLICK = true;
                SELL_MANAGE.SELLRETURNT.IF_DOUBLE_CLICK = true;
                FINANCIAL_MANAGE.ADVANCE_RECEIVABLEST.IF_DOUBLE_CLICK = true;
                FINANCIAL_MANAGE.RECEIVABLEST.IF_DOUBLE_CLICK = true;
                FINANCIAL_MANAGE.RECEIVABLES_ORDERT.IF_DOUBLE_CLICK = true;
                this.Close();
            }
        }
    }
