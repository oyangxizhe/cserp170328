...
{
    public partial class RECEIVABLEST : Form
    {
        ...
        RECEIVABLES F1= new RECEIVABLES();
        ...
        public RECEIVABLEST(RECEIVABLES FRM)
        {
            InitializeComponent();
            F1 = FRM;
        }
        private void RECEIVABLEST_Load(object sender, EventArgs e)
        {
            label14.Text = "(实际应收金额=应收金额-预收金额-扣款金额)";
        ...
        protected void bind()
        {
           ...
           dt = ask(textBox1.Text, comboBox2.Text);
            ...
            dt1 = bc.getdt(cRECEIVABLES.sql + " WHERE 应收单号='" + IDO + "'");
            if (dt1.Rows.Count > 0)
            {

                dateTimePicker1.Text = dt1.Rows[0]["应收日期"].ToString();
                textBox5.Text = dt1.Rows[0]["扣款项目"].ToString();
                textBox6.Text = dt1.Rows[0]["扣款金额"].ToString();
                textBox8.Text = dt1.Rows[0]["实际应收金额"].ToString();
                comboBox3.Text = dt1.Rows[0]["预收单号"].ToString();
        ...
        private DataTable ask(string SRID, string ORID)
        {
            ...
            if (dtx1.Rows.Count > 0)
            {
                ...
                comboBox3.Text = dtx1.Rows[0]["预收单号"].ToString();
...
");
        ...
        {
            bool b = false;
           string GET_IDO=bc.getOnlyString ("SELECT RCID FROM RECEIVABLES_MST WHERE ORID='"+comboBox2 .Text +"'");
            if (IDO == "")
            {
                hint.Text = "编号不能为空！";
                b = true;
            ...
            else if (bc.exists("SELECT * FROM RECEIVABLES_MST WHERE ORID='" + comboBox2.Text + "'") && IDO !=GET_IDO )
            {
                hint.Text = string.Format ("订单号: {0} 已经存在应收款记录！",comboBox2.Text );//一个订单只有一个应收款单
                b = true;
            }
            else if (bc.exists("SELECT * FROM RECEIVABLES_ORDER WHERE RCID='" + IDO + "'"))
            {
                hint.Text = "此应收款单存在收款，不允许修改";
                b = true;
            }
            else if (juage2()==false )
            {
             ...
             else if (comboBox3 .Text  !="" && !bc.exists("SELECT * FROM ADVANCE_RECEIVABLES  WHERE ARID='" + comboBox3 .Text  + "'"))
            {
                hint.Text = "预收款单号不存在于系统中！";
                b = true;
            }
            else if (comboBox3.Text != "" && bc.exists("SELECT * FROM RECEIVABLES_MST  WHERE RCID!='" + textBox1.Text +
                "' AND ARID='" +comboBox3 .Text  + "'"))
            {
                hint.Text = "预收款单已在其它应收款单中存在";
                b = true;
            }
            ...
            {
                hint.Text = "实际应收金额不能为空！";
                b = true;
            }
            else if (bc.yesno(textBox8.Text) == 0)
            {
                hint.Text = "实际应收金额只能为数字且不能为负数！";
                b = true;
            }
        ...
        private void save()
        {
            ...
            btnSave.Focus();
            cRECEIVABLES.MAKERID = LOGIN.USID;
            cRECEIVABLES.RCID = IDO;
            cRECEIVABLES.RECEIVABLES_DATE = dateTimePicker1.Text;
            cRECEIVABLES.ORID = comboBox2.Text;
            cRECEIVABLES.ARID = comboBox3.Text;
            cRECEIVABLES.CUTPAYMENT_PROJECT = textBox5.Text;
            cRECEIVABLES.CUTPAYMENT_AMOUNT = textBox6.Text;
            cRECEIVABLES.INVOICE_NOTAX_AMOUNT = textBox8.Text;
            cRECEIVABLES.save(dt);
            IFExecution_SUCCESS = cRECEIVABLES.IFExecution_SUCCESS;
            hint.Text = cRECEIVABLES.ErrowInfo;
        ...
        private void btnDel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                  if(bc.exists ("SELECT * FROM RECEIVABLES_ORDER WHERE RCID='"+IDO +"'"))
                  {
                      hint.Text = "此应收款单存在收款，不允许删除";
                  }
                  else 
                  {
                    basec.getcoms("DELETE RECEIVABLES_MST WHERE RCID='" + textBox1.Text + "'");
                    basec.getcoms("DELETE RECEIVABLES_DET WHERE RCID='" + textBox1.Text + "'");
        ...
        {
            /*cRECEIVABLES.MAKERID = "";
           
            //cRECEIVABLES.ExcelPrint(dt1, "销货单", System.IO.Path.GetFullPath("销货单.xls"));
