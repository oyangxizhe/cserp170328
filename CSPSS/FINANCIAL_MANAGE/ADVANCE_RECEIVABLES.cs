...
{
    public partial class ADVANCE_RECEIVABLES : Form
    {
        ...
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ADVANCE_RECEIVABLES));
        ...
        public void bind()
        {
            ...
            try
            {
                ...
                stb.Append(cADVANCE_RECEIVABLES.sql);
                stb.Append("  WHERE B.CUID LIKE '%" + textBox1.Text + "%'");
                stb.Append("  AND B.ORID LIKE '%" + textBox3.Text + "%'");
        ...
        public void search_o(string sql)
        {
            ...
            else*/ if (v7 == "Y")
            {
                ...
                dt = bc.getdt(sql+" ORDER BY A.ARID ASC");
 ...
 (SELECT UGID FROM USERINFO WHERE USID='" + LOGIN.USID + "'))" );
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                dtt.Columns.Add("预收单号", typeof(string));
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    dataGridView1["预收日期", i].Value = dt.Rows[i]["预收日期"].ToString();
                    ...
                    dataGridView1["数量", i].Value = dt.Rows[i]["订单数量"].ToString();
                    dataGridView1["识别码", i].Value = dt.Rows[i]["识别码"].ToString();
     
                    dataGridView1["金额", i].Value = dt.Rows[i]["预收金额"].ToString();
                    dataGridView1["总计", i].Value = dt.Rows[i]["总计"].ToString();

               
                    DataTable dtx1 = bc.GET_DT_TO_DV_TO_DT(dtt, "", "预收单号='"+dt.Rows[i]["预收单号"].ToString()+"'");
                    ...
                    else
                    {
                        ...
                        dr["预收单号"] = dt.Rows[i]["预收单号"].ToString();
                        dtt.Rows.Add(dr);
                        j = j + decimal.Parse(dt.Rows[i]["预收金额"].ToString());
                    }
                }
        ...
        {
            CSPSS.FINANCIAL_MANAGE.ADVANCE_RECEIVABLEST FRM = new FINANCIAL_MANAGE.ADVANCE_RECEIVABLEST(this);
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            {
                ADVANCE_RECEIVABLEST FRM = new ADVANCE_RECEIVABLEST(this);
                string v1 = dt.Rows[dataGridView1.CurrentCell.RowIndex]["预收单号"].ToString();
                FRM.IDO = v1;
