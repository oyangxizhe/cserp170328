...
A.SNAME AS 姓名,
C.NODE_NAME AS 作业名称,
CASE WHEN C.OPERATE='Y' AND  C.NODE_NAME NOT IN ('订单作业')  THEN '有权限'
WHEN C.OPERATE='N' AND C.NODE_NAME NOT IN ('订单作业') THEN '无权限'
ElSE ''
END AS 操作权限,
...
ELSE ''
END AS 查询权限,
CASE WHEN C.ADD_NEW='Y' AND  C.NODE_NAME  IN ('订单作业') THEN '有权限'
WHEN C.ADD_NEW='N' AND  C.NODE_NAME  IN ('订单作业') THEN '无权限'
ELSE ''
END AS 新增权限,
CASE WHEN C.EDIT='Y' AND  C.NODE_NAME  IN ('订单作业') THEN '有权限'
WHEN C.EDIT='N' AND  C.NODE_NAME  IN ('订单作业') THEN '无权限'
ELSE ''
END AS 修改权限,
CASE WHEN C.DEL='Y' AND  C.NODE_NAME  IN ('订单作业') THEN '有权限'
WHEN C.DEL='N' AND  C.NODE_NAME  IN ('订单作业') THEN '无权限'
ELSE ''
END AS 删除权限,
