...
namespace XizheC
{
    public class CSELLRETURN
    {
        ...
        private string _SELLRETURN_DATE;
        public string SELLRETURN_DATE
        {
            set { _SELLRETURN_DATE = value; }
            get { return _SELLRETURN_DATE; }
        }
        ...
        private string _SELLRETURN_ID;
        public string SELLRETURN_ID
        {
            set { _SELLRETURN_ID = value; }
            get { return _SELLRETURN_ID; }
        }
...
with ds1 as (SELECT
ROW_NUMBER() OVER (ORDER BY A.SRKEY ASC)  AS  序号,
A.SRID AS 销退单号,
A.ORID as 订单号, 
A.SN as 项次,
B.MODEL as 型号,
...
E.BATCHID AS 识别码,
C.PRICE AS 单价,
CAST(ROUND(E.GECOUNT,2) AS DECIMAL(18,2)) as 销退数量 ,
CAST(ROUND(E.GECOUNT*C.PRICE,4) AS DECIMAL(18,2)) AS 金额,
(
SELECT 
RTRIM(CONVERT(DECIMAL(18,2),SUM(A2.GECOUNT*A3.PRICE))) FROM SELLRETURN_DET A1 
LEFT JOIN GODE A2 ON A1.SRKEY=A2.GEKEY 
...
";
        string setsqlth = @"
UPDATE SELLRETURN_MST SET 
SELLRETURN_DATE=@SELLRETURN_DATE,
REMARK=@REMARK,
DATE=@DATE
";
        string setsqlf = @"
...
";
        ...
        {
            bool b = false;
            if (bc.exists("SELECT * FROM CO_SELLRETURN WHERE ORID='" + ORID + "'"))
            {
                b = true;
        ...
        #region GET_TOTAL_SELLRETURN
        public  DataTable GET_TOTAL_SELLRETURN()
        {
            ...
            dtt.Columns.Add("累计销退数量", typeof(decimal));
            dtt.Columns.Add("订单未结数量", typeof(decimal), "订单数量-累计销退数量+累计销退数量");
            dtt.Columns.Add("状态", typeof(string));
            dtt.Columns.Add("交货日期", typeof(string));

            DataTable dtx1 = bc.getdt("SELECT * FROM SELLRETURN_DET ");
            if (dtx1.Rows.Count > 0)
            {
                for (i = 0; i < dtx1.Rows.Count; i++)
                {
                    DataRow dr = dtt.NewRow();
                    dr["索引"] = dtx1.Rows[i]["SRKEY"].ToString();
                    ...
                    if (dtx1.Rows[i]["SELLRETURNSTATUS_DET"].ToString() == "OPEN")
                    {
                        dr["状态"] = "OPEN";
                    }
                    else if (dtx1.Rows[i]["SELLRETURNSTATUS_DET"].ToString() == "PROGRESS")
                    {
                        dr["状态"] = "部分出货";
                    }
                    else if (dtx1.Rows[i]["SELLRETURNSTATUS_DET"].ToString() == "DELAY")
                    {
                        dr["状态"] = "DELAY";
                    }
                    else
...
");
        ...
        #endregion
        #region GET_SELLRETURN_PROGRESS_COUNT
        public string GET_SELLRETURN_PROGRESS_COUNT(string WAREID,string SRKEY)
        {
            string v = "0";
            DataView dv = new DataView(GET_TOTAL_SELLRETURN());
            dv.RowFilter = "状态 NOT IN ('已出货') AND ID='" + WAREID + "' AND 索引 NOT IN ('"+SRKEY +"')";
        ...
        public bool  JUAGE_REALTY_IFHAVE_SELLCOUNT(string ORID)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    decimal d1 = decimal.Parse(dr["累计销退数量"].ToString());
        ...
        #endregion
        #region JUAGE_SELLRETURN_IF_HAVE_NO_AUDIT
        public bool JUAGE_SELLRETURN_IF_HAVE_NO_AUDIT(string ORID)
        {
            bool b = false;
            string s2 = bc.getOnlyString("SELECT IF_AUDIT FROM SELLRETURN_MST WHERE ORID='" +ORID  + "'");
            if (s2 != "Y")
            {
                b = true;
        ...
        public bool JUAGE_RESIDUE_SECOUNT_IF_LESSTHAN_SR_COUNT(string SRID)
        {
            bool b = false;
            DataTable dt = bc.getdt(sqlo + " WHERE A.SRID='" + SRID + "'");
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    SN = dr["项次"].ToString();
                    decimal d1 = decimal.Parse(dr["销退数量"].ToString());
                    decimal d = 0;
                    decimal d2 = 0;
                    ...
                    if (dtx.Rows.Count > 0)
                    {
                        ...
                        d = decimal.Parse(dtx.Rows[0]["累计销退数量"].ToString());
                        d2 = decimal.Parse(dtx.Rows[0]["累计销退数量"].ToString());
                        if (d - d1 < d2)
                        {
                            b = true;
                            ErrowInfo = "项次:" + SN + " 累计销退数量：" + d.ToString("#0.00") +
                                "与删除的销退数量：" + d1.ToString("#0.00") + "差值：" + (d - d1).ToString("#0.00") +
        ...
        #endregion
        #region save
        public DataTable save(DataTable dt)//返回第一次执行后订单的项次最新库存数量与累计销退数量值
        {
            string year = DateTime.Now.ToString("yy");
            string month = DateTime.Now.ToString("MM");
            string day = DateTime.Now.ToString("dd");
            string varDate = DateTime.Now.ToString("yyy/MM/dd HH:mm:ss").Replace("-", "/");
            if (!bc.exists("SELECT SRID FROM SELLRETURN_DET WHERE SRID='" + SRID + "'"))
            {
            ...
            {
                SQlcommandE_DET(dt, sqlo);
                SQlcommandE_MST(sqlth+" WHERE SRID='"+SRID+"'");
                IFExecution_SUCCESS = true;
                corder.UPDATE_ORDER_STATUS(ORID);
            }
            return dt;
        ...
        protected void SQlcommandE_DET(DataTable dt, string sql)
        {
            ...
            {
                if (dt.Rows[i]["销退数量"].ToString() == "0" || dt.Rows[i]["销退数量"].ToString() == "0.00")
                {
                ...
                else
                {
                    ...
                    SRKEY = bc.numYMD(20, 12, "000000000001", "SELECT * FROM SELLRETURN_DET", "SRKEY", "SE");
                    sqlcom.Parameters.Add("@SRKEY", SqlDbType.VarChar, 20).Value = SRKEY;
                    ...
                    sqlcom.Parameters.Add("@GEKEY", SqlDbType.VarChar, 20).Value = SRKEY;
                    sqlcom.Parameters.Add("@GODEID", SqlDbType.VarChar, 20).Value = SRID;
                    sqlcom.Parameters.Add("@SN", SqlDbType.VarChar, 20).Value = dt.Rows[i]["项次"].ToString();
                    sqlcom.Parameters.Add("@GECOUNT", SqlDbType.VarChar, 20).Value = dt.Rows[i]["销退数量"].ToString();
        ...
        protected void SQlcommandE_MST(string sql)
        {
            ...
            sqlcon.Open();
            sqlcom.Parameters.Add("SRID", SqlDbType.VarChar, 20).Value = SRID;
            sqlcom.Parameters.Add("SELLRETURN_DATE", SqlDbType.VarChar, 20).Value =SELLRETURN_DATE ;
                    ...
                    for (int z = 0; z < totalcount; z++)
                    {
                        ...
                        for (i =i1 ; i < dt.Rows.Count; i++)
                        {
                            ...
                            else
                            {
                                ...
                                worksheet.Cells[6, "J"] = dt.Rows[i]["销退单号"].ToString ();
                                ...
                                worksheet.Cells[8, "J"] = dt.Rows[i]["销退日期"].ToString ();
